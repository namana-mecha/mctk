...
impl GstCamera {
    pub fn new(index: usize, format: Option<CameraFormat>) -> Result<Self, CameraGstError> {
        let camera_format = format.unwrap_or_default();

        if let Err(why) = gstreamer::init() {
            return Err(CameraGstError::InitializeError(why.to_string()));
        }
    ...
    /// open capture stream
    pub fn open_stream(&mut self) -> Result<(), CameraGstError> {
        if let Err(why) = self.pipeline.set_state(State::Playing) {
            return Err(CameraGstError::OpenStreamError(format!(
    ...
    pub fn frame_raw(&mut self) -> Result<Cow<[u8]>, CameraGstError> {
        let bus = match self.pipeline.bus() {
            Some(bus) => bus,
            None => {
                return Err(CameraGstError::ReadFrameError(
        ...
        if let Some(message) = bus.timed_pop(ClockTime::from_seconds(0)) {
            match message.view() {
                ...
                MessageView::Error(err) => {
                    return Err(CameraGstError::ReadFrameError(format!(
                        "Bus error: {}",
                        err.error()
                    )));
                }
    ...
    /// stop device stream
    pub fn stop_stream(&mut self) -> Result<(), CameraGstError> {
        if let Err(why) = self.pipeline.set_state(State::Null) {
            return Err(CameraGstError::StreamShutdownError(format!(
    ...
    pub fn set_camera_format(&mut self, new_fmt: CameraFormat) -> Result<(), CameraGstError> {
    ...
    pub fn compatible_list_by_resolution(
        ...
        let frame_regex = match Regex::new(r"(\d+/1)|((\d+/\d)+(\d/1)*)") {
            Ok(r) => r,
            Err(_why) => return Err(CameraGstError::GeneralError("Regex error".to_string())),
        };
            ...
            Some(c) => {
                for capability in c.iter() {
                    match fourcc {
                        FrameFormat::MJPEG => {
                            if capability.name() == "image/jpeg" {
                                ...
                                let width = match capability.get::<i32>("width") {
                                    Ok(w) => w,
                                    Err(why) => {
                                        return Err(CameraGstError::GetPropertyError {
                                ...
                                for m in frame_regex.find_iter(&value) {
                                    ...
                                    if fraction_string.len() != 2 {
                                        return Err(CameraGstError::GetPropertyError { property: "Framerates".to_string(), error: format!("Fraction framerate had more than one demoninator: {:?}", fraction_string) });
                                    }
                                    ...
                                    if let Some(v) = fraction_string.get(1) {
                                        ...
                                        return Err(CameraGstError::GetPropertyError { property: "Framerates".to_string(), error: "No framerate denominator? Shouldn't happen, please report!".to_string() });
                                    }
                                    ...
                                    if let Some(numerator) = fraction_string.first() {
                                        ...
                                        return Err(CameraGstError::GetPropertyError { property: "Framerates".to_string(), error: "No framerate numerator? Shouldn't happen, please report!".to_string() });
                                    }
                                }
                                resolution_map
                        ...
                        unsupported => {
                            return Err(CameraGstError::NotImplementedError(format!(
...
fn search_device(index: usize) -> Result<(CameraInfo, Option<Caps>), CameraGstError> {
    let device_monitor = DeviceMonitor::new();

    let video_caps = match Caps::from_str("video/x-raw") {
        Ok(cap) => cap,
        Err(why) => {
            return Err(CameraGstError::GeneralError(format!(
    ...
    let _video_filter_id = match device_monitor.add_filter(Some("Video/Source"), Some(&video_caps))
    {
        ...
        None => match device_monitor.add_filter(Some("Source/Video"), Some(&video_caps)) {
            Some(id) => id,
            None => {
                return Err(CameraGstError::StructureError {
        ...
        .find(|(i, _device)| *i == index)
    {
        ...
        None => {
            return Err(CameraGstError::OpenDeviceError(
                index.to_string(),
                "No device".to_string(),
            ))
        }
...
fn generate_pipeline(fmt: CameraFormat, index: usize) -> Result<PipelineGenRet, CameraGstError> {
